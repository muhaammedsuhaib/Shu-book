        ...
        <p className='text-white mt-1'>{userData&&userData.email}</p>
          <MDBDropdown>
                ...
                <MDBDropdownMenu style={{backgroundColor:'white',color:'white'}}>
                  <MDBDropdownItem link  onClick={() => {   toggleOpen(); setOpenNavExternal(!openNavExternal);  }}><MDBIcon fas icon="user-circle"  /> Profile</MDBDropdownItem>
                  {/* <MDBDropdownItem link ><MDBIcon fas icon="cogs" /> Settings</MDBDropdownItem> */}
                  <MDBDropdownItem link onClick={()=>nav('/signup')}><MDBIcon fas icon="user-plus" /> Add new account</MDBDropdownItem>
                  <MDBDropdownItem link onClick={handleLogout}  ><MDBIcon fas icon="sign-out-alt" /> Logout</MDBDropdownItem>
                </MDBDropdownMenu>
              </MDBDropdown>
        </div>
      </MDBCollapse>
...
>
  <MDBModalDialog position='top-right' side>
    <MDBModalContent>
      <MDBModalHeader className='text-white'style={{ background: '#44006b' }}>
      ...
      <MDBModalBody>
        <div className='row'>
          <div className='col-3 text-center'>
            <i className='fas fa-user fa-4x ' style={{ color: '#44006b' }}></i>
          </div>

          <div className='col-9'>
            <p>{userData&&userData.email}</p>
            <p>Welcome, {userData && userData.email.split('@')[0]}! You've almost reached {task.length} tasks added</p>
          </div>
          <div className="container text-end"> <MDBBtn color='tertiary' onClick={()=>nav('/change')}>change pssword</MDBBtn></div>
          <div className="container text-end">
            <img src="/Logo.png" alt="Your Brand Logo" width={'100px'}    style={{ filter: 'brightness(0%)'}}/>
          </div>
        </div>
      </MDBModalBody>
    </MDBModalContent>
